...
export default function FacultyAttendancePage() {
  ...
  function savePeriod(periodId: number) {
    setSaved(prev => ({ ...prev, [periodId]: true }));
    setTimeout(() => setSaved(prev => ({ ...prev, [periodId]: false })), 2500);
  }

  function lockPeriod(periodId: number) {
    setPeriods(prev =>
      prev.map(p => p.id !== periodId ? p : { ...p, locked: true })
    );
  }
                  ...
                  {!period.locked && (
                    <div className="flex gap-3">
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2.5 rounded-2xl text-sm font-bold transition-all duration-200 hover:-translate-y-0.5"
                        style={{
                      ...
                      >
                        <Save size={14} />
                        {saved[period.id] ? "Saved!" : "Save Draft"}
                      </button>
                      <button
                        type="button"
                        onClick={() => { savePeriod(period.id); lockPeriod(period.id); }}
                        disabled={unmarked > 0}
                      ...
                      >
                        <Lock size={14} />
                        Submit & Lock{unmarked > 0 ? ` (${unmarked} left)` : ""}
                      </button>
                    </div>
                  )}
